...
web service plus one free Render Postgres database. There is nothing to paste. `SECRET_KEY`
is generated by Render, and the demo needs no API keys at all.
...
- **The web service sleeps after about 15 minutes idle** and takes roughly a minute to wake on
  ...
  therefore feel slow.
- **Render's free PostgreSQL expires about 30 days after it is created.** To keep the demo free
