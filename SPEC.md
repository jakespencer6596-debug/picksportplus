...
- Post-launch: a global admin manages every league from `/site/leagues`, the only place a `Pool` is created. There they create a league, attach one or more existing users as its initial commissioner(s) by email (only at creation time), or hand out a commissioner invite link at any time, including after the league already exists. Opening that link (`/accept-commissioner?code=...`) works whether or not the recipient already has an account: signed out, it offers sign in or create an account; either path lands them attached as commissioner (or promoted to one, if they were already a plain member) once authenticated. The admin can also "view as commissioner" to enter any league's own commissioner tools (`/league` and everything under it) exactly as that league's commissioner would see them, with a visible "Viewing X as commissioner" banner on every page and a way back out. A global admin is always treated as a commissioner of every pool and can never itself hold a `PoolMember` row; a pool's own commissioner is never a global admin unless their account actually has the global `admin` role. The site admin's own tools (`/site/*`: leagues, provider budgets, mail, contact submissions) are a separate URL namespace from a commissioner's league tools (`/league/*`), and the word "admin" itself never renders on a page a real commissioner reaches, since a pool's own commissioner is not necessarily a site admin. See Section 10 and 10c.
...
- **A save replaces the player's picks for that week in full, never adds to what is already there.** After a save, the database holds exactly the games and confidence values just submitted for that user and week, nothing left over from an earlier save on a game since deselected. This matters because validating the incoming submission is not the same as validating the database afterward: two separately valid submissions (save 15, then save a different 15) can still union into an invalid state if the write path only ever inserts or updates and never deletes what fell out of the newest submission. The write path re-reads what it just wrote before committing and refuses to commit anything that is not exactly `picks_required` picks forming a clean permutation, and a database constraint backs the same rule up at the schema level. See DECISIONS.md, "Orphaned picks", for the incident this closes and PICKS-REPAIR-REPORT.md for the repair.
